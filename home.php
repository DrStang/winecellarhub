...
");
  ...
  ");
    ...
    <section class="grid md:grid-cols-3 gap-6">
        ...
        <button id="addBottleBtn" class="bg-white rounded-2xl shadow p-6 text-left hover:shadow-md transition w-full">
        ...
        <a href="/services/pairing-free" class="block bg-white rounded-2xl shadow p-6 hover:shadow-md transition">
            <h2 class="text-xl font-semibold">🍽️ Pairing</h2>
            <p class="text-gray-600 mt-2">Find wines in your cellar or our catalog for tonight’s menu.</p>
        </a>
    </section>

    <!-- === AI Natural-Language Wine Search (inline widget) === -->
    <section class="mt-6 mb-10 bg-white rounded-2xl shadow p-5">
        <h2 class="text-xl font-semibold mb-2">🔎 AI-powered search of your cellar & our catalog</h2>
        <p class="text-gray-600 text-sm mb-4">
            Try: <em>peppery syrah under $30 to drink this fall</em> · <em>earthy pinot around $40</em> · <em>aged Bordeaux</em>
        </p>
        ...
        <template id="nlq-card-tpl">
            <a class="block rounded-xl border border-gray-200 hover:shadow-md transition overflow-hidden bg-white">
                <div class="flex">
                    <img class="w-20 h-28 object-contain bg-gray-100" />
                    <div class="p-3 flex-1">
                        ...
                        <!-- Hidden by default; shown only if not in inventory -->
                        <div class="mt-2 flex gap-2 hidden action-bar">
                            <form class="inline add-inv" method="POST" action="/move_to_inventory.php">
                            ...
                            <button
                                    class="px-2 py-1 text-xs rounded border border-gray-300 hover:bg-gray-50 add-want">
                                Add to Wantlist
                            </button>
                        </div>
                    </div>
        ...
        (function(){
            ...
            function render(items){
                ...
                if (!items || !items.length) {
                    results.innerHTML = '<div class="text-sm text-gray-500 px-3 py-2">No matches yet. Try adjusting your wording or budget.</div>';
                    return;
                }
                ...
                items.slice(0, 12).forEach(w => {
                    ...
                    // If it's already in the user's inventory, go straight to their bottle page.
                    if (w.in_inventory && w.bottle_id) {
                        a.href = '/bottle.php?id=' + encodeURIComponent(w.bottle_id);
                    } else {
                        // No direct link; reveal quick actions
                        a.removeAttribute('href');
                        const bar = node.querySelector('.action-bar');
                        bar.classList.remove('hidden');

                        // Move to Inventory posts to existing endpoint and redirects back as-is
                        const f = node.querySelector('form.add-inv');
                        f.querySelector('input[name="wine_id"]').value = w.id;

                        // Wantlist: use the existing wantlist_api
                        const wantBtn = node.querySelector('.add-want');
                        wantBtn.addEventListener('click', async (ev) => {
                            ev.preventDefault();
                            wantBtn.disabled = true;
                            try {
                                const resp = await fetch('/wantlist_api.php', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    credentials: 'include',
                                    body: JSON.stringify({ action: 'add', wine_id: w.id })
                                });
                                if (!resp.ok) throw new Error('HTTP ' + resp.status);
                                wantBtn.textContent = 'Added ✓';
                            } catch (e) {
                                console.error(e);
                                wantBtn.textContent = 'Try again';
                            } finally {
                    ...
                    img.src = w.image_url || '/covers/placeholder.jpg';
                    img.alt = (w.name || '') + ' — ' + (w.winery || '');
                    name.textContent = (w.name || '');
                    meta.textContent = [w.winery, w.region, (w.vintage || '')].filter(Boolean).join(' · ');
                    price.textContent = (typeof w.price === 'number') ? `$${w.price.toFixed(2)}` : (w.price || '');
...
<div id="addChooser"
    ...
    <div class="absolute left-1/2 -translate-x-1/2 w-full max-w-lg
        ...
        <div class="mt-4 grid gap-3">
            ...
            <!-- (Optional) Quick last-used action -->
            <button id="optLast"
                    class="hidden w-full justify-center rounded-xl bg-[var(--primary)] text-white py-2 mt-1">
                Use last chosen method
            </button>
        </div>
    </div>
</div>
...
<script>
    (function(){
        ...
        const optMan   = document.getElementById('optManual');
        const optLast  = document.getElementById('optLast');
        ...
        // Remember last used method (scan/manual)
        function saveLast(mode){ try{ localStorage.setItem('last_add_mode', mode); }catch{} }
        function getLast(){ try{ return localStorage.getItem('last_add_mode'); }catch{} return null; }

        if (optScan)  optScan.addEventListener('click', ()=> saveLast('scan'));
        if (optMan)   optMan.addEventListener('click',  ()=> saveLast('manual'));

        // Optional: show a “Use last method” quick button
        const last = getLast();
        if (last && optLast){
            optLast.classList.remove('hidden');
            optLast.textContent = `Use last method (${last === 'scan' ? 'Scan label' : 'Search/manual'})`;
            optLast.addEventListener('click', ()=>{
                if (last === 'scan')  window.location.href = '/scan.php';
                else                  window.location.href = '/add_bottle.php?mode=manual';
            });
        }
    })();
</script>
