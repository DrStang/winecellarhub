...
if($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action']??'')==='add'){
    try{
        ...
        if(!$catalogHadImage && $userUploaded){
            $nameForSlug = ($form['name'] ?? '').' '.($form['winery'] ?? '');
...
<main class="max-w-6xl mx-auto px-4 py-6">
    <header class="mb-4"><h1 class="text-2xl font-extrabold tracking-tight">Add Bottle</h1></header>
    ...
    <form id="addForm" method="post" enctype="multipart/form-data" class="p-4 md:p-6 card">
        ...
        <div class="grid md:grid-cols-3 gap-6">
            ...
            <div>
                <label class="block text-sm" style="color:var(--muted)">Upload bottle image</label>
            ...
            <div id="catalogResultsWrap" class="mt-4 hidden">
                ...
                <div class="overflow-auto mt-2 rounded-lg soft-border">
                    <table id="catalogResults" class="w-full text-sm">
                        <thead>
                        <tr><th class="text-left p-2">Label</th><th class="text-left p-2">Winery</th><th class="text-left p-2">Vintage</th><th class="text-left p-2">Region</th><th class="text-left p-2">Type</th><th class="text-left p-2">Rating</th><th class="text-left p-2">Action</th></tr>
                        </thead>
                        <tbody></tbody>
                    </table>
                </div>
