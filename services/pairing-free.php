...
<div class="max-w-3xl mx-auto p-6">
    ...
    <form method="POST" action="/services/pairing-free_result.php" class="space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label class="block">
                <div class="text-sm text-gray-700 mb-1">Pairing mood</div>
                <select name="mood" class="w-full rounded-lg border px-3 py-2">
                    <option value="classic" <?= isset($_POST['mood']) && $_POST['mood']==='adventurous' ? '' : 'selected' ?>>Classic (safe & balanced)</option>
                    <option value="adventurous" <?= isset($_POST['mood']) && $_POST['mood']==='adventurous' ? 'selected' : '' ?>>Adventurous (zippier, brighter)</option>
                </select>
            </label>


            <label class="flex items-center gap-2 mt-1">
                <input type="checkbox" name="prefer_cellar" value="1" class="rounded" <?= !empty($_POST['prefer_cellar']) ? 'checked' : '' ?>>
                <span>Prefer my cellar first</span>
            </label>
        </div>
