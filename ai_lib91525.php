...
function ai_cache_key_from_row(array $row): string {
    if (!empty($row['wine_id'])) return 'wine:' . (int)$row['wine_id'];
    $parts = [
        trim((string)($row['winery'] ?? '')),
        trim((string)($row['name'] ?? '')),
        trim((string)($row['vintage'] ?? '')),
    ];
    return 'expert:' . sha1(implode('|', $parts));
}
...
function ai_get_cached(PDO $winelist_pdo, string $key): ?string {
    $stmt = $winelist_pdo->prepare("SELECT desc_md FROM wine_ai_cache WHERE cache_key=?");
    $stmt->execute([$key]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    return $row ? (string)$row['desc_md'] : null;
}

function ai_put_cache(PDO $winelist_pdo, string $key, array $row, string $desc): void {
    $ins = $winelist_pdo->prepare("
        INSERT INTO wine_ai_cache (cache_key, wine_id, name, winery, vintage, region, country, grapes, desc_md)
        VALUES (?,?,?,?,?,?,?,?,?)
        ON DUPLICATE KEY UPDATE desc_md=VALUES(desc_md), updated_at=NOW()
    ");
    $ins->execute([
        $key,
        $row['wine_id'] ?? null,
